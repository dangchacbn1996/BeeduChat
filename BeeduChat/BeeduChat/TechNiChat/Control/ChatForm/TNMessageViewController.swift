...
class TNMessageViewController: TNBaseViewController {
    ...
    var btnSend = UIButton(background: Constant.color.btnContinue, corner: 0, border: 0, borderColor: Constant.color.separate, design: nil)
...
extension TNMessageViewController{
    ...
    func UIContactBar(){
        ...
        lblSend.snp.makeConstraints { (maker) in
            maker.left.top.equalToSuperview().offset(Constant.size.paddingView)
        }
        ...
        btnAdd.tintColor = Constant.color.btnTitle
    }
    func UIContent(){
        ...
        btnCalender.snp.makeConstraints { (maker) in
            ...
            maker.left.equalTo(btnAttach.snp.right).offset(4)
            maker.bottom.equalToSuperview().offset(-8)
        }
